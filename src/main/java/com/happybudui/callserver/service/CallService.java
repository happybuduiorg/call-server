...
public class CallService {
    ...
    //获取用户当前的场景
    public ResponseResult<Integer> getUserSceneStatus(String userNumber){
    ...
    //获取用户当前的Location
    public ResponseResult<LocationVo> getUserLocation(String userNumber){
        UserEntity userEntity=userMapper.getUserInfoByNumber(new BigDecimal(userNumber));
        if(userEntity==null){
            return ResultGenerator.error(400,"can't find the number!");
        }
    ...
    //发送通话记录
    public ResponseResult<Integer> pushCallRecord(String callFromNumber,String callToNumber,String callType,
    ...
    //查询自己的接听的通话记录
    public ResponseResult<List<CallRecordEntity>> getCallRecord(String callNumber,String startTime){
    ...
    //发送通话文本信息
    public ResponseResult<Integer> pushCallRecordContent(JSONObject jsonObject){
    ...
    //获取通话详细信息
    public ResponseResult<List<CallRecordContentEntity>> getCallRecordContent(String callRecordId){
